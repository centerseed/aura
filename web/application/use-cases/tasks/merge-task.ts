...
export class MergeTaskUseCase {
  ...
  async execute(
    ...
    // 將資料庫格式 (camelCase + Date) 轉換為領域實體格式 (snake_case + string)
    const sourceReferences: Reference[] = (sourceTask.references || []).map(ref => ({
      id: ref.id,
      type: ref.type,
      content: ref.content,
      title: ref.title,
    ...
    const targetReferences: Reference[] = (targetTask.references || []).map(ref => ({
      id: ref.id,
      type: ref.type,
      content: ref.content,
      title: ref.title,
