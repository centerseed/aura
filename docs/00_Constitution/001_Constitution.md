# Aura System Constitution (亞拉系統大憲法)

本文件是 Aura 專案的「守法原點」。它是超越所有規格書、設計圖與代碼的**最高指導原則**。所有後續的 AI 規劃、開發任務與代碼編修，都必須通過本憲法的合規性檢查 (Constitutional Check)。
...
## 第五條：心理安全與穩定優先
1. **穩定 > 性能**：Aura 的首要任務是提供「穩定感」。任何會導致使用者不可預測焦慮的性能優化或功能，均視為違憲。
